...
mode: wide
sidebarTitle: "What is Uniplex?"
---

# Uniplex is a Passport System for AI Agents.

AI agents are calling APIs and taking actions across the web — but there's no standard way to verify who they are or what they're allowed to do.

**Uniplex is an open protocol that solves this.** It gives agents portable, signed credentials (Passports) that any tool can verify locally — no network calls, no shared secrets, no vendor lock-in.

Think of it as **HTTPS for agent authorization**: a lightweight trust layer that works with your existing infrastructure.
